...
const submitButton = <HTMLButtonElement> document.querySelector('.gr-ext-confirm-button');
const cancelButton = <HTMLButtonElement> document.querySelector('.gr-ext-cancel-button');
const alertSpan = <HTMLSpanElement> document.querySelector('.gr-ext-alert');
const statusTagSpan = <HTMLSpanElement> document.querySelector('.gr-ext-status-display');

/**
 * Gets the value of the checked radio input
 */
function getRadioValue(): number|null {
    const checkedRadio = <HTMLInputElement> document.querySelector('input[name="duration"]:checked');
    if (!checkedRadio) {
        return null;
    }
 ...
 */
function showAlertTag(message: string) {
