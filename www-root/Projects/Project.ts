...
// @ts-ignore
import {CalendarEvent, Email, GmailAdapter, GoogleCalendar, Label} from '../Utilities/lib/ooGAPIAmalgamation.js';

// static GmailAdapter for all Projects
export interface TypeObjectPair {
    type: string;
    id: string;
    name?: string;
}
...
export interface SerialisedProject {
    ...
    colour: string;
    name: string;
    label: TypeObjectPair;
    calendar: TypeObjectPair | null;
}
...
export class Project {
    ...
    private _updateCallback: ((project: Project) => void) | null = null;

    constructor(name: string, colour: string, label: Label, calendar: GoogleCalendar) {
        this.name = name;
        this.colour = colour;
        this.label = label;
    ...
    private _serialiseCalendar(): TypeObjectPair | null {
        let calendarObject: TypeObjectPair | null = null;
        if (this.calendar) {
            calendarObject = {
    ...
    private static _deSerialiseCalendar(calendarObject: TypeObjectPair): GoogleCalendar {
        return new GoogleCalendar(calendarObject.id, calendarObject.name);
    }

    private _serialiseLabel(): TypeObjectPair {
        return {
            type: "gr-gmail-label",
            id: this.label.id,
    ...
    private static _deSerialiseLabel(labelObject: TypeObjectPair): Label {
        return new Label(labelObject.id, labelObject.name);
    }

    static generateFromSerialised(object: SerialisedProject) : Project {

        // extract label and calendar
        const newLabel: Label = this._deSerialiseLabel(object.label);
     ...
     */
    registerModificationCallback(callback: (project: Project) => void) : void {
     ...
     */
    async hasCurrentEvent() : Promise<boolean> {
...
function _eventIsCurrent(event: CalendarEvent): boolean {
