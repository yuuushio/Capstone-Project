...
import type {SerialisedProject} from "./Project.js";
import {Project} from "./Project.js";
// @ts-ignore
import {CalendarAdapter, GDriveAdapter, GmailAdapter, GoogleCalendar, Label} from "../Utilities/lib/ooGAPIAmalgamation.js";
...
class ProjectManagerImpl {
     ...
     */
    private static _getSerialisedListFromLocalStorage(): SerialisedProject[] {
     ...
     */
    getProjectById(targetId: string): Project {
    ...
    private _findIndexOfProjectId(id: string): number {
     ...
     */
    async createProject(name: string, colour: string, createCalendar: boolean = false): Promise<Project> {
     ...
     * @private
     */
    private static async createCalendar(name: string): Promise<GoogleCalendar> {
     ...
     * @private
     */
    private static async createLabel(name: string): Promise<Label> {
     ...
     */
    registerHookFor(project: Project) {

        const callback = async (project: Project) => {
     ...
     * @param project
     */
    async destroyProject(project: Project): Promise<void> {
...
async function getDriveFileContent(): Promise<Object|null> {
...
async function writeDriveFileContent(newContent: object): Promise<void> {
    let file_ids: string[] = await GDriveAdapter.getAppDataFileIDsByName(GDRIVE_FILE_NAME);
    if (file_ids.length === 0) {
